...
prediction at the query step is compared against episode ``y_true``.
    ...
    python scripts/eval_chronos_baseline.py --suite dot-Identifiability-v1 \
        --per-structure 60 --device cuda:0 --out results/reference/chronos_ident.json
"""
...
def _episode_frames(ep, use_covariate):
    ...
    q_idx = int(round(qt * t_len)) if qt <= 1.0 else int(round(qt))
    q_idx = min(max(q_idx, onset), t_len - 1)
    horizon = q_idx - onset + 1
    a_val = (float(ep.intervention.values)
             if isinstance(ep.intervention.values, (int, float)) else float(x[:onset, a].mean()))

    ctx = {"item_id": "ep",
           "timestamp": pd.date_range(_T0, periods=onset, freq=_FREQ),
    ...
    if use_covariate:
        future_df = pd.DataFrame({
            "item_id": "ep",
            "timestamp": pd.date_range(_T0 + pd.Timedelta(seconds=onset),
                                       periods=horizon, freq=_FREQ),
